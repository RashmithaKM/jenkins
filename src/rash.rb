...
class RubySDKHelloWorldPluginApp < AppAbstract
    ...
    def initializeApp()
        ...
        logInit(@log_level)
        log("info", "initializeApp is invoked in jenkins App")     
    end

    def process()
        log("info", "process is invoked in jenkins App")
        saveGreetingMessage(@greeting_param.to_json)
    end  

    def saveGreetingMessage(message)
        log("info", "saveGreetingMessage is invoked in jenkins App")
        log("debug", message)
